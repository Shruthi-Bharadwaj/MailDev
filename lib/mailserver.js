...
const suppressionList = require('../suppression')
...
mailServer.setupOutgoing = function (host, port, user, pass, secure) {
  console.log(`Email relay setup with :: 
  host = ${host},
  port = ${port},
  user = ${user},
  pass = ${pass},
...
function getSubscribedUsers (suppressionList, toAddresses, templateName) {
  return toAddresses.filter((toAddress) => {
    return suppressionList[toAddress.address] && suppressionList[toAddress.address][templateName]
  })
}
...
mailServer.relayMail = function (idOrMailObject, isAutoRelay, done) {
  ...
  mailServer.getRawEmail(mail.id, function (err, rawEmailStream) {
    ...
    console.log('mail == ', mail, mail.to)
    let templateName = 'MyTemplate' // get it from mail.headers
    if (suppressionList && suppressionList.length) {
      mail.to = getSubscribedUsers(suppressionList, mail.to, templateName)
    }
